...
class Maw(G_Object):
    ...
    def _place(self, i = 0):
        ...
        x1,y1 = self.body.GetWorldPoint(fixture.shape.vertices[2])
        x2,y2 = self.body.GetWorldPoint(fixture.shape.vertices[3])
    ...
    def _length(self, pt1, pt2):
        ...
        return sqrt(pow(abs(pt2[0])-abs(pt1[0]),2)+pow(abs(pt2[1])-abs(pt1[1]),2))

    def _alpha(self, A):
        from math import asin
        return asin(self._length(A,(A[1],0))/self._length(A,(0,0)))
    ...
    def draw(self):
        import pygame
        pt = self._place(0)
        pt = self.game.to_screen(( int(pt[0]), int(pt[1])))
        pygame.draw.circle(self.surface.current, (250,250,250),pt, 10, 10)
        G_Object.draw(self)
